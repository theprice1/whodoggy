...
async function main() {
  ...
  const breeds = [
    ...
    "Border Collie", "Australian Shepherd", "Cocker Spaniel", "French Bulldog", "Jack Russell Terrier"
  ];

  for (let i = 1; i <= 500; i++) {
    const selectedRegistry = registries[Math.floor(Math.random() * registries.length)];
  ...
  console.log("WhoDoggy database ready with UK/Northern Ireland microchip registries");
}
