...
router.get("/search/:microchipId", async (req: Request, res: Response) => {
	...
	try {
		console.log(
			`Searching for microchip ${microchipId} across ${registryEndpoints.length} UK registries...`,
		);
		...
		if (match?.data) {
			console.log(
				`Microchip ${microchipId} found in ${match.data.registry.name}`,
			);
			...
			const registryStats = results.reduce(
				(acc, result) => {
...
router.get("/registries", async (req: Request, res: Response) => {
	try {
		...
		res.json({
			...
			systemStatus:
				operational === total ? "fully_operational" : "partial_service",
			registries: statuses,
