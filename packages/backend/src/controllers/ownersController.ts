...
export class OwnersController {
	...
	async getAllOwners(req: Request, res: Response) {
		try {
			...
			const owners: OwnerGroup[] = Object.entries(ownerGroups).map(
				([email, dogsArray]) => {
	...
	async updateOwnerDog(req: Request, res: Response) {
		try {
			...
			if (!ownerEmail || !dogId) {
				return res
					.status(400)
					.json({ error: "Owner email and dog ID are required" });
			}
			...
			const prepareDogUpdate = (dog: Partial<Dog>): Prisma.DogUpdateInput => {
				...
				if (dog.ownerPhone !== undefined)
					validFields.ownerPhone = dog.ownerPhone;
	...
	async validateOwnershipByDog(req: Request, res: Response) {
		try {
			...
			const checkDogOwnership = (
				dog: Pick<Dog, "microchipId" | "ownerEmail">,
			): boolean => {
	...
	async getAggregatedData(req: Request, res: Response) {
		try {
			...
			// Function that was causing the error - now properly typed
			const processedStats = rawStats.map(
				(stat: { createdAt: Date; _count: { id: number } }) => ({
