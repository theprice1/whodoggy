...
export const searchByOwner = async (
  ...
  try {
    ...
    if (!ownerEmail && !ownerName && !ownerPhone) {
      res.status(400).json({
        error: "At least one owner parameter is required (email, name, or phone)"
      });
      return;
    }
    ...
    if (ownerEmail && typeof ownerEmail === "string") {
      orConditions.push({ ownerEmail: { contains: ownerEmail, mode: "insensitive" } });
    }

    if (ownerName && typeof ownerName === "string") {
      orConditions.push({ ownerName: { contains: ownerName, mode: "insensitive" } });
    }
