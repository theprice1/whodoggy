...
// Test the connection
export const _testConnection = async (): Promise<void> => {
	try {
		await prisma.$connect();
		console.log("✅ Database connected successfully via Prisma");

		// Test query to verify connection
		const _result = await prisma.$queryRaw`SELECT NOW() as current_time`;
...
// Graceful shutdown
export const _closeConnection = async (): Promise<void> => {
	try {
