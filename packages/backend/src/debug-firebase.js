...
const _credentialsJson = process.env.FIREBASE_ADMIN_CREDENTIALS_JSON;
...
try {
	const _credentials = JSON.parse(credentialsJson);
	...
	if (credentials.private_key) {
		...
		const _hasProperStart = credentials.private_key.includes(
			"-----BEGIN PRIVATE KEY-----",
		);
		const _hasProperEnd = credentials.private_key.includes(
			"-----END PRIVATE KEY-----",
		);
		const _hasNewlines = credentials.private_key.includes("\n");
		...
		if (!hasNewlines) {
			console.log(
				"ℹ️  Private key appears to be on one line (this might be the issue)",
			);
		}
	} else {
