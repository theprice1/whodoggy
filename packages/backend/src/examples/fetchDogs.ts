...
async function fetchDogsByRegistry(registryId: number): Promise<void> {
  ...
  try {
    ...
    res.rows.forEach((dog: any) => {
      console.log(`  🐕 ${dog.name} (${dog.breed}) - Microchip: ${dog.microchip_id} - Registry: ${dog.registry_name}`);
    });
...
async function main(): Promise<void> {
  try {
    ...
    const testRes = await testPool.query("SELECT microchip_id FROM dogs LIMIT 1");
    await testPool.end();
