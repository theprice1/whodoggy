...
export const _validateSearchInput = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    _microchipSchema.parse(req.body);
    next();
  } catch (error) {
    if (error instanceof ZodError) {
      const _formattedErrors = error.issues.map((e: ZodIssue) => ({
        path: e.path.join("."),
        message: e.message,
      }));
      return res.status(400).json({ errors: _formattedErrors });
    }
...
export const _validateDogInput = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    _dogSchema.parse(req.body);
    next();
  } catch (error) {
    if (error instanceof ZodError) {
      const _formattedErrors = error.issues.map((e: ZodIssue) => ({
