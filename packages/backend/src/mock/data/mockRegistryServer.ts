...
export async function createMockRegistryServer(
	...
	app.get("/api/search/owner", (req: Request, res: Response) => {
		...
		const results = mockDogs.filter((dog: MockDog) => {
			const nameMatch = name
				? dog.ownerName.toLowerCase().includes((name as string).toLowerCase())
				: true;
			const emailMatch = email
				? dog.ownerEmail.toLowerCase().includes((email as string).toLowerCase())
				: true;
			const phoneMatch = phone
				? dog.ownerPhone.includes(phone as string)
				: true;
	...
	return new Promise<void>((resolve, reject) => {
		const server = app.listen(port, () => {
			console.log(
				`✅ Mock registry server '${dataFileName}' running on port ${port}`,
			);
		...
		server.on("error", (err: Error) => {
			console.error(
				`❌ Failed to start registry '${dataFileName}' on port ${port}:`,
				err,
			);
			reject(err);
		});
...
export async function createRegistryServer(
	...
	try {
		...
		console.log(
			`🏥 ${registryName} registry (ID: ${registryId}) started on port ${port}`,
		);
	} catch (error) {
