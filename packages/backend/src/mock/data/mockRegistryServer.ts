...
export async function createMockRegistryServer(port: number, dataFileName: string) {
  const app = express();
  app.use(cors());
  ...
  app.get('/api/microchip/:id', (req, res) => {
    ...
    const dog = mockDogs.find((d: any) => d.microchipId === id);
  ...
  return new Promise<void>((resolve) => {
    app.listen(port, () => {
      console.log(`Mock registry server running on port ${port} with data from ${dataFileName}`);
      resolve();
    });
  });
}
