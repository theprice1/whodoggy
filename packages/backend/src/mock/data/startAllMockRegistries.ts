...
async function startAllMockRegistries(): Promise<void> {
	console.log(
		`Starting ${numberOfRegistries} Northern Ireland microchip registry mock servers...`,
	);
	console.log(
		`Port range: ${startPort} to ${startPort + numberOfRegistries - 1}`,
	);
	...
	try {
		for (let i = 0; i < numberOfRegistries; i++) {
			...
			console.log(
				`Starting ${registryName} on port ${port} with data file: ${dataFileName}`,
			);
		...
		console.log(
			`All ${numberOfRegistries} mock registry servers started successfully!`,
		);
		...
		console.log(
			"  GET  /api/dogs                   - Get all dogs in registry",
		);
...
async function testAllRegistries(): Promise<void> {
	...
	results.forEach((result, index) => {
		if (result.status === "fulfilled") {
			...
			failCount++;
			console.log(
				`Registry ${registryNames[index]} (port ${startPort + index}) failed: ${result.reason}`,
			);
		}
	});
...
async function testRegistryEndpoint(
	...
	try {
		...
		throw new Error(
			`${registryName} on port ${port} is not responding: ${error}`,
		);
	}
}
...
async function simulateCrossRegistrySearch(microchipId: string): Promise<void> {
	...
	for (let i = 0; i < numberOfRegistries; i++) {
		...
		searchPromises.push(
			searchRegistryForMicrochip(port, registryName, microchipId),
		);
	}
	...
	results.forEach((result, index) => {
		...
		if (result.status === "fulfilled" && result.value) {
			...
			console.log(
				`🎯 FOUND in ${registryName}: ${JSON.stringify(result.value, null, 2)}`,
			);
		}
	});
...
async function searchRegistryForMicrochip(
	port: number,
	registryName: string,
	microchipId: string,
): Promise<any> {
	try {
		const response = await fetch(
			`http://localhost:${port}/api/microchip/${microchipId}`,
		);
		if (response.ok) {
...
if (require.main === module) {
	startAllMockRegistries()
		.then(() => {
			...
			console.log(
				"   - Individual registry: curl http://localhost:4101/health",
			);
			console.log(
				"   - Cross-registry search: Use your WhoDoggy app or test script",
			);
