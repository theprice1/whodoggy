...
export const startAllRegistries = async (): Promise<void> => {
  try {
    console.log(`🚀 Starting ${totalRegistries} Northern Ireland Microchip Registry APIs...`);
    ...
    console.log(`✅ All ${totalRegistries} registry APIs started successfully!`);
    console.log(`📋 Registry APIs running on ports ${basePort + 1} to ${basePort + totalRegistries}`);
...
const testSingleRegistry = async (port: number, registryId: number): Promise<void> => {
  try {
