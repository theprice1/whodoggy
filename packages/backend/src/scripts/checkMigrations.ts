...
const run = (): void => {
	...
	console.log(
		`Target database: ${process.env.PGDATABASE}@${process.env.PGHOST}`,
	);
	console.log(
		"This will show what migrations would be applied without actually running them.\n",
	);
	...
	migrate.on("close", (code: number | null) => {
		if (code === 0) {
			...
			console.log(
				"The above output shows what migrations would be applied to your WhoDoggy database.",
			);
		} else {
			console.error(`\nMigration check process exited with code: ${code}`);
			console.error(
				"Please review any errors above and check your database connection.",
			);
		}
		process.exit(code ?? 1);
	});
	...
	migrate.on("exit", (code: number | null, signal: string | null) => {
		if (signal) {
			console.log(
				`Migration check process was terminated by signal: ${signal}`,
			);
		}
	});
};
...
const listMigrations = (): void => {
	...
	const listCheck: ChildProcess = spawn(
		"pnpm",
		["exec", "node-pg-migrate", "ls", "-c", "./node-pg-migrate.config.js"],
		{
			stdio: "inherit",
			env: process.env,
		},
