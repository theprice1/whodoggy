...
const requiredEnvVars = ['DATABASE_URL', 'PGHOST', 'PGUSER', 'PGDATABASE', 'PGPASSWORD', 'PGPORT'];
const missingVars = requiredEnvVars.filter(varName => !process.env[varName]);

if (missingVars.length > 0) {
  console.error(`Missing required environment variables for migration: ${missingVars.join(', ')}`);
...
const rollbackMigration = (): void => {
  ...
  const rollback: ChildProcess = spawn(
    "pnpm",
    ["exec", "node-pg-migrate", "down", "1", "-c", "./node-pg-migrate.config.js"],
    {
      stdio: "inherit",
      env: process.env,
    },
