...
async function seedDogs(): Promise<void> {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    console.log('💡 Generate mock data first by running: npm run generate-mock-data');
    return;
  }
  ...
  try {
    ...
    console.log(`✅ Created/verified ${Object.keys(registries).length} registries`);
    ...
    await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      for (let i = 0; i < dogs.length; i += batchSize) {
        ...
        for (const dog of batch) {
          ...
          if (dog.registryName && typeof dog.registryName === 'string') {
            ...
            if (registryValues.length > 0) {
              const randomIndex = Math.floor(Math.random() * registryValues.length);
        ...
        // Progress update
        console.log(`📊 Processed ${Math.min(i + batchSize, dogs.length)}/${dogs.length} dogs...`);
      }
    });

    console.log(`✅ Seeding completed! Added ${processedCount} dogs to the database.`);
