...
export const getAllDogs = async (): Promise<DogWithRegistry[]> => {
  try {
...
export const getDogById = async (id: number): Promise<DogWithRegistry | null> => {
  try {
...
export const getDogByMicrochipId = async (microchipId: string): Promise<DogWithRegistry | null> => {
  try {
    const dog = await prisma.dog.findUnique({
      where: {
        microchipId,
...
export const createDog = async (dogData: CreateDogData): Promise<DogWithRegistry> => {
  try {
    // First validate that the registry exists
    const registryExists = await prisma.registry.findUnique({
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Unique constraint')) {
        throw new Error(`A dog with microchip ID ${dogData.microchipId} already exists`);
      }
      throw error;
    }
...
export const updateDog = async (id: number, updateData: UpdateDogData): Promise<DogWithRegistry | null> => {
  try {
    ...
    if (error instanceof Error) {
      // Handle unique constraint violations
      if (error.message.includes('Unique constraint')) {
        throw new Error(`A dog with microchip ID ${updateData.microchipId} already exists`);
      }
      throw error;
    }
...
export const deleteDog = async (id: number): Promise<DogWithRegistry | null> => {
  try {
...
export const searchDogs = async (searchTerm: string): Promise<DogWithRegistry[]> => {
  try {
    const dogs = await prisma.dog.findMany({
      where: {
        OR: [
...
export const getDogsByRegistry = async (registryId: number): Promise<DogWithRegistry[]> => {
  try {
    const dogs = await prisma.dog.findMany({
      where: {
        registryId,
